...
def F_measure(cm):
    ...
    p = np.mean(ps).round(4)
    r = np.mean(rs).round(4)
    # f_seen = np.mean(fs[:-1]).round(4)
    # f_unseen = round(fs[-1], 4)
    result = {'F1': f, 'Precision': p, 'Recall': r}
    # result = {'Known': f_seen, 'Open': f_unseen, 'F1-score': f}
    return result


def best_threshold(data, y_pred, y_true, num):
    criterion = [i * 0.01 for i in range(20, 30)]
    best_results = {'Known': 0, 'Open': 0, 'F1-score': 0, 'Accuracy': 0, 't': 0}
    for k, t in enumerate(criterion):
        y_pred_fuben = y_pred
        for i in range(len(data)):
            if data.cpu().detach().numpy().min(axis=1)[i] > t:
                y_pred_fuben[i] = num
        cm = confusion_matrix(y_true, y_pred_fuben)
        results = F_measure(cm)
        acc = round(accuracy_score(y_true, y_pred_fuben) * 100, 2)
